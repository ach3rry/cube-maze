...
- 教学面：熟悉滚动、碰墙和进入终点暗井。
- 第 1 关：纯迷宫路线。
- 第 2 关：加入静态蓝色陷阱洞，掉入任意陷阱后返回第 1 关。
...
- 点击“开始挑战”时解锁移动端音频；支持 `navigator.vibrate` 的浏览器会同时触发碰撞震动。
...
iOS Safari 网页端不提供标准硬件震动接口，因此会使用低频音效和画面抖动作为碰撞反馈。Android 等支持 `navigator.vibrate` 的浏览器会保留原生震动。
